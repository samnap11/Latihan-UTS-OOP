...
private:
    BaseException* exc;
public:
    TransactionFailedException(BaseException* exc_in) {
        exc = exc_in;
    }
    void printMessage() {
...
private:
    string number;
public:
    AccountNotFoundException(string num) {
        number = num;
    } 
    void printMessage() {
...
public:
    ...
    int findAccountIdx(string number) {
        ...
        if(idx == -1) {
            AccountNotFoundException* t = new AccountNotFoundException(number);
            throw t;
        }
        return idx;
    ...
    void transfer(string fromNumber, string toNumber, int amount) {
        ...
        try {
            ...
            TransactionFailedException* t = new TransactionFailedException(e);
            throw t;
        }
